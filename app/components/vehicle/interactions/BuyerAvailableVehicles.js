...
class BuyerAvailableVehicles extends React.Component {
  ...
  buyNowVehicle(vehicleId, amount) {
    $.ajax({
      ...
      window.history.back();
      this.props.history.pushState(null, '/buyer/purchases');
      ...
      <div>
          ...
          <div className="modal-dialog" role="document">
            <div className="modal-content">
              ...
              <div className="modal-footer">
                ...
                <button type="button" className="btn btn-primary" data-dismiss="modal" onClick={this.onClickBuyNow.bind(this)}>Confirm Buy now</button>
              </div>
            </div>
          </div>
        </div>
        ...
        {(() => {
          switch (this.props.vehicle.status) {
            case 'DRAFT':
              return <div>Draft (not for sale)</div>;
            case 'IN_AUCTION':
            ...
            case 'SOLD_AUCTION_PENDING_APPROVAL':
              return <div>Sold with pending approval from Seller</div>;
            default:
